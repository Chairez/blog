...
class EliminarArticuloSeltest < Test::Unit::TestCase
  ...
  def test_eliminar_articulo_sel
    ...
    @driver.find_element(:id, "post_titulo").send_keys "a eliminar"
    @driver.find_element(:id, "post_texto").clear
    @driver.find_element(:id, "post_texto").send_keys "a eliminar"
    @driver.find_element(:name, "commit").click
    assert !60.times{ break if (element_present?(:link, "Volver") rescue false); sleep 1 }
    @driver.find_element(:link, "Volver").click
    assert !60.times{ break if (element_present?(:xpath, "//tr[12]/td") rescue false); sleep 1 }
    @driver.find_element(:xpath, "(//a[contains(text(),'Eliminar')])[11]").click
    assert_match /^¿Eliminar artículo[\s\S]$/, close_alert_and_get_its_text()
  end
